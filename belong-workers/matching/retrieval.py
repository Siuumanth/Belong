...
class CandidateRetriever:
    ...
    def _get_effective_options(self, custom_options: Optional[RetrievalOptions] = None) -> Dict[str, Any]:
        ...
        return {
            "candidate_pool_limit": opts.candidate_pool_limit or getattr(settings, "MATCHING_CANDIDATE_POOL_LIMIT", 50),
            "pre_rank_limit": opts.pre_rank_limit or getattr(settings, "MATCHING_PRE_RANK_LIMIT", 15),
            "max_distance_km": opts.max_distance_km or getattr(settings, "MATCHING_MAX_DISTANCE_KM_DEFAULT", 100),
            "min_similarity_threshold": (
                opts.min_similarity_threshold 
                if opts.min_similarity_threshold is not None 
                else getattr(settings, "MATCHING_MIN_SIMILARITY_THRESHOLD", 0.0)
            ),
            "bidirectional_weight": (
                opts.bidirectional_weight 
                if opts.bidirectional_weight is not None 
                else getattr(settings, "MATCHING_BIDIRECTIONAL_WEIGHT", 0.5)
            ),
