...
It's built as a technical learning project focused on **AI/RAG pipelines, LangGraph agents, embeddings, and pgvector retrieval**, rather than as a production dating product.
...
- **Go** owns the gateway, authentication, and routing.
    
- **Python** is a single modular-monolith service owning all Belong domain logic — onboarding, extraction, embeddings, retrieval, and compatibility reasoning.
    
- The Python service is split into an **API process** for fast requests and a **worker process** for long-running LLM/embedding workloads.
    
- The API and workers use a **Postgres-backed job queue**, avoiding an external broker for the MVP.
    
- **Matching is asynchronous**: a match request creates a job → a worker claims it → hard filtering → pgvector retrieval → pairwise LLM reasoning → ranking → results persisted → client polls for completion.
